...
class CNN_number_classifier:
    ...
    def __init__(self):
        ...
        W_fc1 = self.weight_variable([15 * 10 * 64, 3024])
        b_fc1 = self.bias_variable([3024])
        ...
        W_fc2 = self.weight_variable([3024, 10])
        ...
        # tf.sign()
        #self.accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
        self.train_step = tf.train.AdamOptimizer(1e-5).minimize(self.l2_loss)
        self.sess= tf.InteractiveSession()
        self.sess.run(tf.initialize_all_variables())
    def fit(self,X,Y,n=2000):
        for i in range(n):
            # batch = mnist.train.next_batch(50)
            batch = np.random.randint(X.shape[0], size=1)
            batch_data = X[batch]
